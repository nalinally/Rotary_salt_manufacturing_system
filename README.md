...
ロータの形状や回転数を変えながら海水濃縮実験を行いました。

まず、研究室内で１号機をぶん回しました。

結果として、140分回して海水濃度 3.5% --> 3.5% でした。参考として、水を装置と同じ大きさの容器に入れて335分放置した場合、塩分濃度は3.5% --> 3.6%(水の蒸発量でいうと111mlくらい) になりました。
...
何となく海水がうまく濃縮される条件がわかったので、ここからはロータの形状を変えながら実験を行い、より良いロータの形状を探します。
